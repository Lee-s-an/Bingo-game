...
int main()
{
	...
	while (true)
	{
		...
		case AM_EASY:
			...
			break;
		case AM_HARD:
			//하드모드는 현재 숫자중 빙고줄 완성 가능성이 가장높은 줄을
			...
			//모든 라인을 조사했으면 ILine에 가능서이 가장 높은 줄 번호가
			//저장되었다.
			...
			{
				//세로줄일 경우 iLine 5~9사이의 값이다.
				for (int i = 0; i < 5; ++i)
				{
					if (iAINumber[i * 5 + (iLine - 5)] != INT_MAX)
					{
						ilnput = iAINumber[i * 5 + (iLine - 5)];
						break;
					}
				}
			}
